...
// Stored procedures
const sqlGetAll = `CALL usp_listarbarberos()`;
const sqlFind = `CALL usp_buscarbarberos(?)`;
const sqlInsert = `CALL usp_insertarbarbero(?,?,?,?,?)`;
const sqlUpdate = `CALL usp_actualizarbarberos(?,?,?,?,?)`;
const sqlDelete = `CALL usp_eliminarbarbero(?)`;
...
app.get('/barberos/:codbarbero', function (req, res) {
    connection.query(sqlFind, [req.params.codbarbero], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
app.post('/barberos', function (req, res) {
    ...
    connection.query(sqlInsert,
        ...
        params.descripcion,
        params.correo,
        params.fotobarbero], function (error, results, fields) {
            if (error) throw error;
            res.end(JSON.stringify(results));
        });
});
